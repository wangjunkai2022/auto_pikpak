...
class ShanYouXiang(MailBase):
    def __init__(self):
        super().__init__()

    def _get_pikpak_codes(self, email_user, email_password):
        folders = ["INBOX", "Junk"]
        mails = []
        try:
            # 连接 IMAP 服务器
        ...
        return self._get_pikpak_codes(email_user, email_password)
    ...
    def get_one_mail(self):
        ...
        if mail_type != "":
            try:
                ...
                self.update_json_file(json_data)
                return datas[0]
            except Exception as e:
