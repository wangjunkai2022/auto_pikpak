...
# 创建随机UA
def get_user_agent(client_id, device_id, device_id2):
    ...
    # return ua
    t = time.time()
    ua = f"ANDROID-com.pikcloud.pikpak/1.42.6 accessmode/ devicename/Samsung_Sm-g988n appname/android-com.pikcloud.pikpak appid/ action_type/ clientid/{client_id} deviceid/{device_id} refresh_token/ grant_type/ devicemodel/SM-G988N networktype/WIFI accesstype/ sessionid/ osversion/7.1.2 datetime/{int(round(t * 1000))} protocolversion/200 sdkversion/2.0.1.200200 clientversion/1.42.6 providername/NONE clientip/ session_origin/ devicesign/div101.{device_id}{device_id2} platformversion/10 usrno/"
    print(ua)
    return ua
...
def send_code(client_id, device_id, email, user_agent):
    captcha_token = initCaptcha(client_id, device_id, email, user_agent).get("captcha_token")
...
def initCaptcha(client_id, device_id, email, user_agent, captcha_token=""):
    url = "https://user.mypikpak.com/v1/shield/captcha/init"
    payload = {
        "client_id": client_id,
        "action": captcha_token == "" and "POST:/v1/auth/verification" or "POST:/v1/auth/signin",
        "device_id": device_id,
        "captcha_token": captcha_token,
...
def start(email, password):
    ...
    user_agent = get_user_agent(client_id1, device_id, device_id2)
    sendData = send_code(client_id1, device_id, email, user_agent)
    print(sendData)
    # # 开始模拟请求
