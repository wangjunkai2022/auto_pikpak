...
logger = logging.getLogger("captcha_kill")
...
def solvev2e_reg(url: str = "", sitekey=sitekey_login):
    params_dict = extract_parameters(url)
    url = remove_parameters(url)
    solvev2e_url = "https://captchakiller1.p.rapidapi.com/solvev2e"
    querystring = {"site": f"{url}",
                   "sitekey": f"{sitekey}", "gdomain": "false", "invisible": "false"}
    headers = {
...
def captcha_rewardVip(sitekey: str = sitekey_rewardVip):
